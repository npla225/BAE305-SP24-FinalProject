...
After completing the testing phase of the project there were several aspects of the current design that could be modified to improve the efficiency of the system. To begin, it would be recommended that the solenoids be replaced with pumps to ensure accurate dispensation. The calculated elevation head did not supply enough flow to dispense on time. This required the tanks to be placed on a shelf ~2ft above the machine, reducing practicality. By using pumps, there wouldn't be a reliance on gravity to dispense the drink and the rate of dispensation would not change based on the volume of fluid in the system. The pumps would also reduce the need to seal the tubes to the caps and not require the tanks to be flipped upside down, reducing the risk of leaks while pouring and spillage from refilling the containers.  The next possible modification to the machine would be to make the stirring rod height adjustable. In the current design, the stirring rod has to be detachable and placed into the cup before filling to be able to fit underneath the canopy of the machine. to require less user input in the mixing process, it would be recommended that the stirring motor be put on a vertical track system attached to another motor so that the rod could be raised and lowered after the cup is placed under the nozzle. This would require another motor, (two are included in the SparkFun kit), a rack and pinion system, and a new function to be created in the code that would lower the motor after the cup is placed and modify the "end()" function to raise the rod after the mixing is complete.  
## Testing Results Discussion ##
The capabilities of the mixer are described with the mixer being able to dispense two different liquid components and mix 4-ounce, 8-ounce, and 12-ounce of a drink. This design mixer could be implemented in small restaurants and bars to allow for ease of use mixing for customers. The mixer works but due to using gravity to create head pressure, the flow of the liquid is slow and therefore the mixer may not be capable of being used in larger establishments. The limitations of the mixer include slow flow, inability to dispense any liquid amount of 12 ounces, and ability to mix two different types of liquid. The mixer cannot change any of the properties of the mixing agents or add any sugar or cream into the drink without assistance from a human.

The elevation head was too inconsistent when the mixer ran and needed to have the tubing filled with liquid up to the solenoid. Once the solenoid is opened to allow liquid to pass through, the mixer will dispense liquid at a consistent flow. However to fill the tubing, multiple test runs needed to be completed to achieve the desired amount of liquid in the tubing. By implementing a pump on both sides of the machine, this problem could be fixed and a greater and more consistent flow could be possible. However, our group did not have the budget or expertise necessary to implement pumps in our design leading to us discovering the issue in testing.
